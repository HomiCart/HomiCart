function GenerateRoutesFromColumnA() {
  ...
  // Group every 10 stops into one route link.
  // The stops are ALREADY in optimal visit order (nearest-neighbor + 2-opt
  // starting from the depot), so each group naturally continues from where the
  // previous one ended. The depot (start point) is NOT added as a stop — it is
  // only the origin used to compute the order, never a numbered delivery.
  const groupSize = 10;
  const output = [];
  let routeNumber = 1;
  ...
  for (let start = 0; start < coords.length; start += groupSize) {
    ...
    output.push([
      ...
      group.length,
      routeLink,
      `Stops ${start + 1} → ${start + group.length}`
    ]);
